            ...
            if self.observing == false {
                ...
                postsRef.observe(.childAdded, with: { snapshot in
                    ...
                    if let uid = Auth.auth().currentUser?.uid {
                        ...
                        // コメントの処理
                        // 初期化
                        postData.comments.removeAll()
                        for commentSnapshot in snapshot.childSnapshot(forPath: "comments").children {
                            let commentData = CommentData(snapshot: commentSnapshot as! DataSnapshot)
                            postData.comments.insert(commentData, at: 0)
                        }
